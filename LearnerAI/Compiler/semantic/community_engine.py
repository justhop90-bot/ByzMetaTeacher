...
def default_community_engine_registry() -> CommunityEngineSemanticsRegistry:
    ...
    practices = (
        ...
        EnginePractice(
            "providers.world-availability",
            "provider-state",
            "Provider availability is a world-state capability observation distinct from command feasibility.",
            EvidenceClass.ENGINE_FACT,
            PracticeStatus.PARTIAL,
            (aoe2ai, duke),
            "Represent provider availability as explicit world-state evidence; do not infer it from can-* alone.",
            "A provider can disappear or become unusable while the strategic demand that depended on it remains valid.",
        ),
        EnginePractice(
            "queue.admission-capacity",
            "queue-state",
            "Production commands admit work through provider-specific queue capacity; queue acceptance is not object birth.",
            EvidenceClass.COMMUNITY_PRACTICE,
            PracticeStatus.EVIDENCE_ONLY,
            (duke, aoe2ai),
            "Keep queue admission, pending work, and object birth distinct until native queue-state semantics are mapped.",
            "A production request may remain queued or fail to admit without proving that the target object exists.",
        ),
        EnginePractice(
            "resource-control.escrow",
            "resource-control",
            "Escrow/resource-control can reserve resources for a specific execution purpose and must not replace strategic ownership.",
            EvidenceClass.COMMUNITY_PRACTICE,
            PracticeStatus.PARTIAL,
            (duke, "repo://AiByz/03_HD_ARCHAEOLOGY/PASS52_HD_RESOURCE_ESCROW_PRODUCTION_SUSTAINMENT_ARCHAEOLOGY_2026-09-04.md"),
            "Represent escrow as a transient execution commitment with an explicit release boundary.",
            "Protected resources remain unavailable to competing consumers until the native or compiler-local commitment releases them.",
        ),
        EnginePractice(
            "objects.birth-completion",
            "object-lineage",
            "For asynchronous production, object birth is the world-state boundary that separates queued work from an existing entity.",
            EvidenceClass.ENGINE_FACT,
            PracticeStatus.PARTIAL,
            ("repo://AiByz/03_HD_ARCHAEOLOGY/PASS72_OBJECT_BIRTH_LINEAGE_2026-09-05.md", "repo://AiByz/03_HD_ARCHAEOLOGY/PASS80_AGGREGATE_PRODUCTION_OBSERVABILITY_AND_COMMAND_SUCCESS_2026-09-05.md"),
            "Use object-birth evidence only where the native observation actually identifies the requested entity.",
        ...
        EnginePractice(
            ...
            EvidenceClass.COMPILER_POLICY,
            PracticeStatus.CONTRACTED,
            ("repo://AiByz/03_HD_ARCHAEOLOGY/PASS65_PRODUCTION_FAILURE_QUEUE_SATURATION_RECOVERY_2026-09-04.md", "repo://AiByz/03_HD_ARCHAEOLOGY/PASS83_FAILURE_TO_EXECUTION_FEEDBACK_2026-09-05.md"),
            "Return failed execution to reassessment rather than silently cancelling the original strategic identity.",
            "Native .per provides world-state evidence rather than a generic action-failure callback.",
        ),
        EnginePractice(
