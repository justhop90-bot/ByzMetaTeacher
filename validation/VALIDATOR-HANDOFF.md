...
3. **Rule too long** means a parsed DE `defrule` exceeds the documented 32-element rule limit. The count is every nested parenthesized command/fact/logical form inside the rule, excluding the outer `defrule` wrapper. Exactly 32 is legal; greater than 32 is a hard failure. The current Basilisk controller has no 32-element rules; six rules sit at 31 elements, leaving one element of DE headroom. Those remain semantic hotspots, not automatic defects.
