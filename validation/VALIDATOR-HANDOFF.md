...
Current source baseline: Basilisk/Basilisk.per is at controller content SHA `4cd2f2707c84de48018c24a712abccedde613ef8` with 795 parsed rules. Source-level validation of the current controller reports a 239-character maximum line and a 31-element maximum rule size, with no rules at the 32-element hard ceiling. The validator now resolves symbolic object-family operands through AIRef object names/lines, inventory-derived wildcard sets, local defconsts, documented parameter values, and class symbols where UnitId accepts class wildcards. This closes the bare `siege-tower` identifier gap without rejecting legitimate `trebuchet-set`, villager wildcard sets, or `infantry-class` usages. The exact current front-door validator source was executed in-memory against the complete checked-in controller and supporting inventories and passed every validator check. The connected game machine remains unavailable, so this is validator/source verification rather than game/runtime PASS. Current validator SHA is `59d1aa326b7a17f8cbb22f274f541c8fd3dd2f98`; current self-test SHA is `c595cef15c4119aa62354626f3582a85e7b0a65b`.
