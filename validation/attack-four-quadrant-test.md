...
The Basilisk-specific choice is to classify structural progress first, then use relative force to split non-positive progress into STALLED versus REASSESS. That keeps the mechanism small and falsifiable. The attack measurement window is 180 seconds to stay close to practical timer-driven attack cadence rather than making each attack cycle a five-minute lock. A separate severe-collapse path triggers retreat when the existing reserve/force threshold is breached at measurement time, so the four-quadrant test does not confuse ordinary inferiority with an emergency combat collapse.

Separate control case: deliberately let relative force fall below the severe-collapse floor during an active attack. Expected behavior is attack-now reset, retreat, STALLED result, standing-army demand, and retreat cooldown rather than ordinary building-delta cadence.
