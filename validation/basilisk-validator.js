...
function validateResourceModeArbiter(rules) {
  ...
  const imperialPrereqOverride = explicitOverrides.filter(({ rule }) =>
    rule.includes("(set-goal bt-resource-mode-goal bt-resource-mode-imperial-prereq)"),
  );
  const unauthorizedOverrides = explicitOverrides.filter(
    ({ rule }) =>
      !rule.includes("(set-goal bt-resource-mode-goal bt-resource-mode-imperial-prereq)"),
  );
  assert.equal(
    imperialPrereqOverride.length,
    1,
    "[Resource mode] exactly one Imperial-prerequisite override must bypass the mode-0 idle gate",
  );
  assert.equal(
    unauthorizedOverrides.length,
    0,
    "[Resource mode] an unrelated mode writer bypasses the mode-0 idle gate",
  );
  assert.ok(
    imperialPrereqOverride[0].rule.includes("(current-age == castle-age)") &&
      imperialPrereqOverride[0].rule.includes("(building-type-count-total castle < 1)") &&
      imperialPrereqOverride[0].rule.includes("(not (can-research-with-escrow imperial-age))") &&
      imperialPrereqOverride[0].rule.includes("(building-type-count-total monastery >= 1)") &&
      imperialPrereqOverride[0].rule.includes("(building-type-count-total university >= 1)") &&
      imperialPrereqOverride[0].rule.includes("(building-type-count-total siege-workshop >= 1)") &&
      imperialPrereqOverride[0].rule.includes("(not (goal bt-resource-mode-goal bt-resource-mode-food-crisis))") &&
      imperialPrereqOverride[0].rule.includes("(not (goal bt-resource-mode-goal bt-resource-mode-gold-crisis))") &&
      imperialPrereqOverride[0].rule.includes("(not (goal bt-resource-mode-goal bt-resource-mode-wood-crisis))"),
    "[Resource mode] Imperial-prerequisite override must explicitly yield to all P0 crises",
  );
